...
function(input, output, session) {
  ...
  lapply(1:simLength, function(i){
    ...
    assign(paste0("totalForage", i), reactive({
    ...
    output[[paste0("cowSell", i)]] <- renderUI({
      ...
      # }
             ...
             #  bsButton("herdetails", label = "", icon = icon("question"), style = "info", class="quest", size = "extra-small"),
             #  bsPopover(id = "herdetails", title = "Cattle in herd", content = paste0("Keep in mind that yearlings (weaned calves that are not yet producing calves) aren't counted in these herd size numbers. You do not have the option to sell yearlings in this game. These herd size predictions also assume that you go back to 'normal' culling and calf sale rates next year. For these reasons, your herd may not go all the way to 0 if you sell off all of your cows and calves."),                        placement = "top", 
             # trigger = "hover", 
             # options = list(container = "body")))
      ...
      if(input[[paste0("insuranceDeposit", i)]] != ""){
        ...
        fluidRow(
          ...
          if(myOuts[i, assets.cash] + indem[[i]]$indemnity - 
             indem[[i]]$producer_prem - input[[paste0("d", i, "adaptExpend")]] > 0){
      ...
      if(!is.null(input[[paste0("year", i, "Summer")]])){
        if(input[[paste0("year", i, "Summer")]] == 1){
          ...
          herdy2 <- shinyHerd(herd_1 = herdy1,  # t-1 for year 2 is next years herd size
                              ...
                              calves_2 = (floor(herdy0 * AdjWeanSuccess(get(paste0("totalForage", i))(), T, simRuns$normal.wn.succ, 1)) - calves),  # Calves in the herd this year minus those that are sold via the slider input
                              deathRate = simRuns$death.rate)  
